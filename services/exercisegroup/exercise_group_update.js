...
function updatingExerciseGroupInDatabase (request, response) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const validationParams = await Joi.validate(request.params.exercisegroupId, schema.exercise_group_update.requestParams)
      const validationBody = await Joi.validate(request.body, schema.exercise_group_update.requestBody)

      const update = exerciseGroupUpdate(request.params.exercisegroupId, validateToken.relation_id, request.body)

      resolve(response
        .status(200)
        .send({ message: creation.message })
      )
    } catch (error) {
